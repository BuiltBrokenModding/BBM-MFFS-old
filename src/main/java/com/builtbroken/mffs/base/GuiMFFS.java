...
public class GuiMFFS<MACHINE extends TileMFFS> extends GuiContainerBase
{
    ...
    protected void drawFortronText(int x, int y)
    {
        if (tile instanceof TileFortron)
        {
            ...
            drawTextWithTooltip("fortron", ChatFormatting.WHITE + "" + new UnitDisplay(UnitDisplay.Unit.LITER, (fortronTile.getFortronEnergy() / 1000)).symbol() + "/" + new UnitDisplay(UnitDisplay.Unit.LITER, (fortronTile.getFortronCapacity() / 1000)).symbol(), 35, 119, x, y);
        }
    }
}